...
#method swim, run, etc, retorna el result con el evento especificado (swim, bike, etc.). Si no existe lo crea y lo retorna
...
#crea el setter (this.swim= (build event))
#assign event details to result
  define_method("#{name}=") do |event|
    event=self.send("#{name}").build_event(event.attributes)
end
#expone en esta clase los getters setters de tipo swim_secs, swim_pace100, etc, usando el getter/setter creado previamente
#y asignando el valor que se le pasa en el caso del setter
#expose setter/getter for each property of each result (swim_secs, swim_pace100, bike_mph, t1_secs, etc)
  ...
  def update_total(event)
  	self.secs = 0
  	results.each {|r| self.secs += r.secs.nil? ? 0 : r.secs }
  	#self.secs = results.sum(:secs)
  end

  def the_race
  	race.race #race=raceRef embedded .race --> La race concreta 
  end
end
